...
Use `./start.sh` to start the server - this loads environment variables from `.env` (including VAPID keys for push notifications) and sets up Tailscale serve for HTTPS.
...
If Claude gets killed ("zsh: killed claude") while using Tether mobile:

1. **Check system logs immediately:**
   ```bash
   log show --predicate 'processImagePath contains "claude"' --last 5m
   ```

2. **Check Tether server logs for input sent before kill:**
   ```bash
   tmux capture-pane -t lifeos:2.0 -p -S -2000 | grep 'data.*==' | \
     sed 's/.*"data" => "\([^"]*\)".*/\1/' | \
     while read b64; do
       echo -n "$b64 -> "
...
3. **Look for control characters in decoded input:**
   - `03` = Ctrl+C (SIGINT)
   - `1c` = Ctrl+\ (SIGQUIT)
   - `7f` = DEL/backspace (normal)
   - `0d` = Enter (normal)
   - `1b5b...` = escape sequences (normal)
...
**Resolved issues:**
- SGR mouse events from touch taps were being sent to Claude Code TUI, causing accidental clicks (e.g., tapping to open keyboard could click "cancel" in Claude). Fixed by filtering mouse events in `terminal.js` (2026-01-26).
